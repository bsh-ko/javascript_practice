...
/**
 * 모든 클래스 내에 생성자는 필수! 생성자가 없는 클래스는 존재할 수 없다
 * new User() 객체 생성 구문 -> 생성자 호출 구문
 * 위처럼 개발자가 명시적으로 클래스 내에 생성자를 추가하지 않았다면
...
class User2 {
     ...
     * 생성자 함수는 어디선가 new로 객체 생성 시점에 객체 생성 역할을 하는 것이 중요하므로
     * 아래처럼 {} 로직을 가지지 않는 생성자도 그 자체로 의미가 있다.
     * 원한다면 {} 에 로직을 추가해서 객체 생성과 동시에 개발자의 로직이 실행되도록 할 수 있고
     */
...
/**
 * 자동으로 추가되는 생성자는 매개변수 없는 생성자이다.
 * 객체 생성하면서 데이터를 전달받아서 생성되어야한다면, 개발자가 명시적으로 생성자를 선언한다.
 */
...
//하나의 클래스 내에 생성자 여러 개 선언
//==> 불가하다.. 클래스 내에 명시적으로 생성자 선언은 하나만
class User4 {
    constructor(){}
    constructor(name){}
}
